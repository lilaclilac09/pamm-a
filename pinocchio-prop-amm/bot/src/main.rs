...
async fn main() -> Result<()> {
    ...
    // If any task exits (only happens on halt or panic), log and exit.
    tokio::select! {
        r = oracle  => { tracing::error!("oracle_arm exited: {:?}",  r); }
        r = trading => { tracing::error!("trading_arm exited: {:?}", r); }
        r = metrx   => { tracing::error!("metrics exited: {:?}",     r); }
    }
