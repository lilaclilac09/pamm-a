...
//
// ── Instruction layouts ───────────────────────────────────────────────────────
// Compute-swap (tags 0/1):
//   [0]: side  [1..9]: input  [9..17]: rx  [17..25]: ry  [25..1049]: storage
// After-swap (tag 2) — DIFFERENT layout from compute-swap:
//   [0]: tag  [1]: side  [2..10]: input  [10..18]: output
//   [18..26]: post_rx   [26..34]: post_ry  [34..42]: step  [42..1066]: storage
//   After-swap returns only [u8; 40] (first 40 bytes); executor preserves rest.
//
// ── Fee formula ───────────────────────────────────────────────────────────────
...
//   → below normalizer 30 bps floor, wins retail flow
// After 0.5%+ price shock (8 steps @ 4 bps):    8 + 6 + 32 = 46 bps
//   → stays elevated while arbs continue attacking
// Sustained arb-directional flow:               +25 bps on that side
//   → taxes arbs detected via direction EMA
...
const SHOCK_FEE_PER_STEP: u64   = 400_000;     // 4 bps × remaining steps

const DIR_ALPHA_1E9: u128       = 300_000_000; // direction EMA α=0.30
const DIR_ONE_M_ALPHA: u128     = 700_000_000;
const DIR_HIGH: u64             = 700_000_000; // bullish arb signal
const DIR_LOW: u64              = 300_000_000; // bearish arb signal
const MOMENTUM_FEE_1E9: u64     = 2_500_000;   // 25 bps surcharge on arb side
...
pub fn process_instruction(
    ...
    match instruction_data[0] {
        ...
        2 => {
            if let Some(new_state) = after_swap(instruction_data) {
                set_return_data_bytes(&new_state);
            }
        }
...
pub fn compute_swap(data: &[u8]) -> u64 {
    ...
    // Clamp stored values to valid range to guard against uninitialized storage
    ...
    // Momentum surcharge only after first trade (last_rx > 0 means we have history)
...
// Update and return the 40-byte state from an after_swap callback.
// Returns None if instruction_data is too short to be valid.
// Only returns 40 bytes; executor preserves storage bytes 40..1024 unchanged.
fn after_swap(data: &[u8]) -> Option<[u8; 40]> {
    // after_swap layout: tag(1)+side(1)+input(8)+output(8)+rx(8)+ry(8)+step(8)+storage(1024)
    // storage starts at byte 42; we need bytes [42..82] for our 40-byte state
    if data.len() < 82 { return None; }

    let cur_rx = read_u64_raw(data, 18) as u128;
    let cur_ry = read_u64_raw(data, 26) as u128;

    // Read our state from correct storage offset (byte 42)
    let old_vol       = read_u64_raw(data, 42);
    let last_rx       = read_u64_raw(data, 50);
    let last_ry       = read_u64_raw(data, 58);
    let shock_steps   = read_u64_raw(data, 66).min(SHOCK_DECAY_STEPS);
    let direction_ema = read_u64_raw(data, 74).min(1_000_000_000);

    // Relative price change: |Δ(ry/rx)| / (ry/rx) = |cur_ry*old_rx - old_ry*cur_rx| / (old_ry*cur_rx)
    // Use saturating_mul to prevent u128 overflow with extreme reserve values
    let price_change_1e9: u64 = if last_rx > 0 && last_ry > 0 && cur_rx > 0 {
        let old_rx    = last_rx as u128;
        let old_ry    = last_ry as u128;
        let cross_new = cur_ry.saturating_mul(old_rx);
        let cross_old = old_ry.saturating_mul(cur_rx);
        let diff  = if cross_new > cross_old { cross_new - cross_old }
                    else                     { cross_old - cross_new };
        let denom = old_ry.saturating_mul(cur_rx);
        if denom > 0 {
    ...
    // Shock countdown: reset on large move, decay otherwise
    ...
    // Direction EMA: infer from whether X decreased (buy) or increased (sell)
    let new_dir = if last_rx > 0 {
        let dir_signal: u128 = if cur_rx < last_rx as u128 {
            1_000_000_000  // X bought → bullish
        } else if cur_rx > last_rx as u128 {
            0              // X sold → bearish
        } else {
            500_000_000    // unchanged → neutral
        };
        ((DIR_ALPHA_1E9  * dir_signal
        + DIR_ONE_M_ALPHA * direction_ema as u128)
        / 1_000_000_000) as u64
    } else {
        500_000_000  // first trade: seed neutral
    };

    let mut state = [0u8; 40];
    write_u64(&mut state, 0,  new_vol);
    write_u64(&mut state, 8,  cur_rx as u64);
    write_u64(&mut state, 16, cur_ry as u64);
    write_u64(&mut state, 24, new_shock);
    write_u64(&mut state, 32, new_dir);
    Some(state)
}
...
// Read u64 from a raw byte slice (instruction_data) at an absolute offset
#[inline]
