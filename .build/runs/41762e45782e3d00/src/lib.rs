...
//
// At calm (vol ≈ 0.1%, no shock):  8 + 2×1 ≈ 28 bps
//   → stays below normalizer's 30 bps floor → attracts retail
// After 0.5%+ price shock:         8 + 6 + 32 = 46 bps for 8 steps
//   → holds elevated fee while arbs continue attacking same move
// Sustained directional flow:      +25 bps on arb-favored side
//   → taxes identified arbs without raising retail fees
...
pub fn process_instruction(
    ...
    match instruction_data[0] {
        ...
        2 => {
            // BPF path: read state from storage snapshot embedded in instruction_data[42..],
            // return updated 40-byte state via return data so the executor writes it back.
            if instruction_data.len() >= 82 {
                let cur_rx = read_u64_raw(instruction_data, 18);
                let cur_ry = read_u64_raw(instruction_data, 26);
                let new_state = new_state_from_storage(cur_rx, cur_ry, &instruction_data[42..]);
...
pub fn compute_swap(data: &[u8]) -> u64 {
    ...
    // Clamp to valid ranges to be robust against uninitialized storage
    ...
    // Bullish arb (ema > 0.70) buys X (side=0) → surcharge side=0
    // Bearish arb (ema < 0.30) sells X (side=1) → surcharge side=1
...
// ── After-swap: update state from post-trade reserves ────────────────────────

// Native ABI: called directly by the test harness.
// data = encoded after_swap instruction (rx at [18..26], ry at [26..34])
// storage = live mutable storage; read old state from here, write new state here.
pub fn after_swap(data: &[u8], storage: &mut [u8]) {
    ...
    let new_state = new_state_from_storage(cur_rx, cur_ry, storage);
    let write_len = storage.len().min(40);
    storage[..write_len].copy_from_slice(&new_state[..write_len]);
}

// ── Shared state update ───────────────────────────────────────────────────────

// Compute new 40-byte state from post-trade reserves and current state bytes.
// state_bytes is either live storage (native path) or instruction_data[42..] (BPF path).
fn new_state_from_storage(cur_rx: u64, cur_ry: u64, state: &[u8]) -> [u8; 40] {
    let old_vol       = if state.len() >= 8  { read_u64(state, 0) } else { 0 };
    let last_rx       = if state.len() >= 16 { read_u64(state, 8) } else { 0 };
    let last_ry       = if state.len() >= 24 { read_u64(state, 16) } else { 0 };
    let shock_steps   = if state.len() >= 32 { read_u64(state, 24).min(SHOCK_DECAY_STEPS) } else { 0 };
    let direction_ema = if state.len() >= 40 { read_u64(state, 32).min(1_000_000_000) } else { 500_000_000 };

    // Relative price change: |Δ(ry/rx)| / (ry/rx)
    // Use saturating_mul to guard against overflow with extreme reserve values
    ...
    let mut out = [0u8; 40];
    write_u64(&mut out, 0,  new_vol);
    write_u64(&mut out, 8,  cur_rx);
    write_u64(&mut out, 16, cur_ry);
    write_u64(&mut out, 24, new_shock);
    write_u64(&mut out, 32, new_dir);
    out
}
